...
echo'
    ...
    if($account_type=='creator' || $account_type=='listener' || $account_type=='moderator' || $account_type=='premium_listener')echo' <a href="library.php"><button class="button">Library</button></a>';
    if($account_type=='creator' || $account_type=='listener' || $account_type=='moderator' || $account_type=='premium_listener')echo' <a href="history.php"><button class="button">History</button></a>';
    if($account_type=='creator' || $account_type=='moderator')echo' <a href="upload_songs.php"><button class="button">Upload</button></a>';
      echo '</aside>
    <br><br><br><br>
        <div id="content">
        <h1>SONGS YOU HAVE RECENTLY LISTENED TO</h1>
        <div class="kon">';
    try {
    ...
    join (select song_id, date_time
    from user_listened_to_song
    where username like '" . $pfpname."') as timetbl
    on song.id=timetbl.song_id
	...
	order by timetbl.date_time desc;");
    ...
    echo '      <table style="width:60%;position:absolute;top:20%;left:-10%;border: 2pt solid black;border-collapse: collapse;">
              <tr style="border: 2pt solid black;border-collapse: collapse;">
                <th class="photo" style="border: 2pt solid black;border-collapse: collapse;">Photo</th>
                ...
                <th style="border: 2pt solid black;border-collapse: collapse;">Views</th>
				<th style="border: 2pt solid black;border-collapse: collapse;">Listening price</th>
				<th style="border: 2pt solid black;border-collapse: collapse;">Download price</th>
    ...
    echo '<td style="border: 2pt solid black;border-collapse: collapse;">' .$nrviews. '</td>
	<td style="border: 2pt solid black;border-collapse: collapse;">' .$tempdata2['listening_price']. '</td>
	<td style="border: 2pt solid black;border-collapse: collapse;">' .$tempdata2['download_price']. '</td>';
...
catch(PDOException $e)
{
    ...
    $query13 = "UPDATE user_listened_to_song SET date_time = '$date' WHERE username LIKE '$curusername' AND song_id = '$playingSong'";
        $conn->query($query13);
	...
	echo'
		...
		<div style="width:90%;position:relative;left:5%;background-color:black;height:20pt;border-top-right-radius: 25px;border-bottom-left-radius: 25px;border-bottom-right-radius: 25px;border-top-left-radius: 25px;">
